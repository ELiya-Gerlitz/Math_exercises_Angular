...
export class AllQuestionsService {
  ...
  private emitListClickSubject = new Subject<boolean>()
  private ans : number | string = "?"
  private progress : number = 0
  public index : number

  get emitListClick$(){
    return this.emitListClickSubject.asObservable()
  }
  ...
  get questionsArr$(){
    return this.questionArraySubject.asObservable() 
  }
  get studentsAnswer$(){
...
public emitListClick(){ //when the list is being clicked, it lets the options know, in order to enable new clicks no the options.
  this.emitListClickSubject.next(false)
}
